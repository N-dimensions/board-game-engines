...
int reinforce(unsigned int *k, const vector<int>& choices, unsigned int movepl, unsigned int *holon, float *boardstoreprob[], unsigned int *boardhistory)
{
	...
	{	
		...
		if(win(k1, empty)==0)
		{
			boardstoreprob[holon[movepl]][1]=boardstoreprob[holon[movepl]][1]+60;
			for(p=0;p<=movepl;p++)
			{
				boardstoreprob[boardhistory[p]][1]=boardstoreprob[boardhistory[p]][1]+60;
			}
		...
		if(win(k1, empty)==1)
		{
			boardstoreprob[holon[movepl]][0]=boardstoreprob[holon[movepl]][0]+60;
			boardstoreprob[holon[movepl]][1]=boardstoreprob[holon[movepl]][1]+60;
			for(p=0;p<=movepl;p++)
			{
				boardstoreprob[boardhistory[p]][0]=boardstoreprob[boardhistory[p]][0]+60;
		...
		if(win(k1, empty)==2)
		{
			boardstoreprob[holon[movepl]][0]=boardstoreprob[holon[movepl]][0]+30;
			boardstoreprob[holon[movepl]][1]=boardstoreprob[holon[movepl]][1]+60;
			for(p=0;p<=movepl;p++)
			{
				boardstoreprob[boardhistory[p]][0]=boardstoreprob[boardhistory[p]][0]+30;
		...
		{	
			...
			while(count!=1000)																		//play 30 pseudo Random games
			{
...
{
	unsigned int i, boardbest, boardposnew, besttemp;
	besttemp=0;
	cout<<"Thinking of best for:- "<<movepl<<endl; 
	boardbest=boardpos+pow(3,choices[0])*(2-movepl%2);
	...
	for(i=0;i<choices.size();i++)
	{
		...
		cout<<"Move in mind and chances : "<<choices[i]<<"-->"<<boardstoreprob[boardposnew][0]<<"/"<<boardstoreprob[boardposnew][1]<<endl;
		//cout<<boardposnew<<endl;
		if(movepl%2==1)
		{
